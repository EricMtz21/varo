...
const inputCls = "w-full bg-[#141D2E] border border-[#1E2D45] rounded-xl px-4 py-3 text-[#E2E8F0] text-sm outline-none focus:border-[#818CF8] transition-colors placeholder:text-[#3D5070]";
const selectCls = "bg-[#141D2E] border border-[#1E2D45] rounded-xl px-3 py-3 text-[#E2E8F0] text-sm outline-none focus:border-[#818CF8] transition-colors cursor-pointer";
    ...
    <div className="flex gap-1.5 flex-wrap">
      {options.map(([key, label]) => (
        <button key={key} type="button" onClick={() => onChange(key)}
          className={`px-3 py-1.5 rounded-xl text-xs font-bold transition-colors cursor-pointer border ${value === key ? activeClass : "bg-[#141D2E] text-[#475569] border-[#1E2D45] hover:text-[#E2E8F0] hover:border-[#334155]"}`}>
          {label}
        </button>
      ))}
    </div>
...
export default function EditTransactionModal({ tx, onSave, onClose }) {
  ...
  // scope: "single" for non-recurring, "one"/"forward"/"all" for recurring
      ...
      >
        ...
        <div className="p-5 sm:p-6">
          ...
          <div className="flex items-center justify-between mb-5">
            <h2 className="font-bold text-lg text-[#E2E8F0]">Editar movimiento</h2>
          ...
          {isRecurring && (
            <div className="mb-5 bg-[#141D2E] rounded-xl p-3 border border-[#1E2D45]">
          ...
          <div className="flex bg-[#141D2E] rounded-xl p-1 mb-5 border border-[#1E2D45]">
            <button type="button" onClick={() => handleTypeChange("expense")}
              className={`flex-1 py-2.5 rounded-lg text-sm font-bold transition-all cursor-pointer ${form.type === "expense" ? "bg-[#F87171] text-[#07090F] shadow-sm" : "text-[#475569] hover:text-[#E2E8F0]"}`}>
              Gasto
            </button>
            <button type="button" onClick={() => handleTypeChange("income")}
              className={`flex-1 py-2.5 rounded-lg text-sm font-bold transition-all cursor-pointer ${form.type === "income" ? "bg-[#34D399] text-[#07090F] shadow-sm" : "text-[#475569] hover:text-[#E2E8F0]"}`}>
              Ingreso
            </button>
          </div>
          ...
          <div className="space-y-3">
            ...
            <div>
              <p className="text-[11px] text-[#475569] font-bold uppercase tracking-widest mb-2">Categoría</p>
                ...
                {categories.map((cat) => {
                  ...
                  return (
                    <button key={cat} type="button" onClick={() => setField("category", cat)}
                      className={`flex flex-col items-center gap-1.5 py-2.5 px-1 rounded-xl text-[10px] font-bold transition-all cursor-pointer border ${selected ? "border-transparent" : "bg-[#141D2E] text-[#475569] border-[#1E2D45] hover:text-[#CBD5E1] hover:bg-[#1A2537]"}`}
            ...
            <div>
              ...
              <input type="date" value={form.date}
                onChange={(e) => setField("date", e.target.value)} className={inputCls} />
            </div>
          </div>
            ...
            <div className="mt-5 pt-4 border-t border-[#1E2D45]">
              <button type="button"
                ...
                <div className="text-left">
                  <p className="text-sm font-bold text-[#E2E8F0]">Repetir movimiento</p>
                ...
                <div className={`w-11 h-6 rounded-full relative transition-colors shrink-0 ml-3 ${form.recurring.enabled ? "bg-[#818CF8]" : "bg-[#1E2D45]"}`}>
              ...
              {form.recurring.enabled && (
                <div className="mt-4 space-y-4">
                  <div>
                    <label className="text-[11px] text-[#475569] font-bold uppercase tracking-widest mb-2 block">Frecuencia</label>
                    <PillGroup options={Object.entries(FREQ_LABELS)} value={form.recurring.frequency}
                      onChange={(v) => setRecurring("frequency", v)}
                      activeClass="bg-[#818CF8] text-[#07090F] border-transparent" />
                  </div>
                  <div>
                    <label className="text-[11px] text-[#475569] font-bold uppercase tracking-widest mb-2 block">Termina</label>
                  ...
                  {form.recurring.endType === "date" && (
                    <input type="date" value={form.recurring.endDate} min={form.date}
                      onChange={(e) => setRecurring("endDate", e.target.value)} className={inputCls} />
                  )}
                  {form.recurring.endType === "occurrences" && (
                    <div className="flex items-center gap-3">
                      <span className="text-sm text-[#64748B] whitespace-nowrap">Después de</span>
